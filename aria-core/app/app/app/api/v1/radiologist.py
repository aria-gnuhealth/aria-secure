...
async def reject_analysis(
    ...
    try:
        from app.utils.email import send_email
        if doctor:
            html = "<html><body><h2>Analyse rejetee</h2><p>Motif: " + reason + "</p><p>Dr. " + current_user.first_name + " " + current_user.last_name + " a rejete votre analyse.</p></body></html>"
            send_email(doctor.email, "ARIA - Analyse rejetee par le radiologue", html)
    except Exception as e:
        print(f"Email rejet: {e}")
