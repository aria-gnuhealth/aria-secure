...
export default function RegisterScreen() {
  ...
  const [form, setForm] = useState({ first_name: "", last_name: "", email: "", password: "", role: "doctor" });
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Alert.alert("Compte créé !", "Un email de vérification a été envoyé.", [
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <TextInput style={styles.input} placeholder="Prénom" value={form.first_name} onChangeText={v => setForm({ ...form, first_name: v })} />
        <TextInput style={styles.input} placeholder="Nom" value={form.last_name} onChangeText={v => setForm({ ...form, last_name: v })} />
        ...
        <View style={styles.rolesContainer}>
          {roles.map(role => (
            <TouchableOpacity key={role} style={[styles.roleBtn, form.role === role && styles.roleBtnActive]} onPress={() => setForm({ ...form, role })}>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: "#fff", borderWidth: 1, borderColor: "#ddd", borderRadius: 10, padding: 14, fontSize: 16, marginBottom: 16, color: "#333" },
  label: { fontSize: 14, color: "#555", marginBottom: 10, fontWeight: "500" },
  rolesContainer: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 24 },
  roleBtn: { borderWidth: 1, borderColor: "#ddd", borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: "#fff" },
  ...
  button: { backgroundColor: "#1a73e8", borderRadius: 10, padding: 16, alignItems: "center", marginBottom: 16 },
